...
import com.mobileer.oboetesteradded.miditools.MidiOutputPortConnectionSelector;
import com.mobileer.oboetesteradded.miditools.MidiPortConnector;
import com.mobileer.oboetesteradded.miditools.MidiTools;
...
public class TapToToneActivity extends TestOutputActivityBase {
    ...
    private class MyTestListener implements AudioMidiTester.TestListener {
        @Override
        public void onTestFinished(final AudioMidiTester.TestResult result) {
    ...
    // Runs on UI thread.
    private void showTestResults(AudioMidiTester.TestResult result) {
        String text;
        int previous = 0;
        if (result == null) {
            text = "";
